...
app.post("/poll", async (req, res, next) => {
  ...
  try {
    ...
    const poll = await Poll.create({
      question,
      user,
      options: options.map((option) => ({ option, votes: 0,votedBy: [], })),
      users: [],
    });
    ...
    const emails1= cust.map((e)=>e.email)
    const names1=cust.map((e)=>e.name)
    let email=emails1
    let name=names1
    console.log(email)
    console.log(name)
     for(var i=0;i<email.length;i++){
        ...
        var mailOptions={
            ...
            text:'Hi '+name[i]+','+'\nNew Poll updated in the application !!!\n'+'\nTitle:'+question
        }
...
app.delete("/poll/:id", async (req, res, next) => {
    ...
    try {
      // if (answer) {
        const result = await Poll.deleteOne({_id:pollId});
        console.log(result)
         var response = result;
          return res.status(202).json(response);
    } catch (err) {
